<div class="scrim" on:click|self={() => dispatch('close')} role="dialog" aria-modal="true" aria-label="Settings">
  <div class="sheet">
    ...
    <div class="body">
      <div class="grid2">
        ...
        <div class="field">
          ...
          <div class="note">“info” shows the handshake progress in the core log.</div>
        </div>
      </div>
      ...
      <div style="display:grid;gap:9px;margin-top:14px">
        ...
        <button class="toggle" class:on={s.allowInsecureAll} on:click={() => (s.allowInsecureAll = !s.allowInsecureAll)} type="button">
          <span class="switch" /><span class="txt"><b>Allow insecure TLS certificates</b>
            <span>Needed for self-signed servers. Off by default: it disables certificate verification for every node.</span></span>
        </button>
        ...
        <button class="toggle" class:on={s.connectOnStartup} on:click={() => (s.connectOnStartup = !s.connectOnStartup)} type="button">
          <span class="switch" /><span class="txt"><b>Reconnect the last node on launch</b>
            <span>Starts the tunnel a moment after the window opens.</span></span>
        </button>
        <button class="toggle" class:on={s.autoPingOnStart} on:click={() => (s.autoPingOnStart = !s.autoPingOnStart)} type="button">
          <span class="switch" /><span class="txt"><b>Test ping on launch</b>
            <span>Latency numbers are ready when the list is.</span></span>
        </button>
      </div>
    ...
    <footer>
      ...
      <button class="btn primary" on:click={() => dispatch('save', s)}>{@html icons.check} Save</button>
    </footer>
  </div>
</div>
...
<script>
  ...
  const defaults = {
    socksPort: 10808, httpPort: 10809, pingTimeoutMs: 3000, pingSamples: 3,
    systemProxy: true, proxyBypass: '127.0.0.1;localhost;192.168.*;<local>', bypassLocal: true,
    allowInsecureAll: false, sniffing: true, logLevel: 'warning', dnsServers: '1.1.1.1,8.8.8.8',
